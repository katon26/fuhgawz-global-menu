 ...
 * userSwitcher.js - Implements a macOS-style user switcher button for the system menu.
 */
...
export const UserSwitcherButton = GObject.registerClass(
  { GTypeName: 'FUHGlobeUserSwitcherButton' },
  class UserSwitcherButton extends PanelMenu.Button {
    _init(extension) {
      ...
      this._buttonIcon = new St.Icon({
        ...
        icon_size: 18,
        style_class: 'fuhgawz-user-switcher-button',
      });
      this.add_child(this._buttonIcon);

      if (this.menu?.actor) {
        this.menu.actor.add_style_class_name('fuhgawz-user-switcher-menu');
    ...
    async _rebuildMenu() {
      ...
      if (users.length === 0) {
        ...
        placeholder.actor.add_style_class_name('fuhgawz-user-switcher-empty');
    ...
    _createUserWidget(user, currentUserName, sessionInfo) {
      ...
      const avatarBin = new St.Bin({
        style_class: 'fuhgawz-user-card-avatar-frame',
        x_expand: true,
        y_expand: true,
      });
      ...
      const avatar = new UserAvatar(user, {
        styleClass: 'fuhgawz-user-card-avatar',
      ...
      if (isCurrent || isSignedIn) {
        const sessionBadge = new St.Icon({
          style_class: isCurrent
            ? 'fuhgawz-user-session-badge current-user'
            : 'fuhgawz-user-session-badge',
      ...
      const nameLabel = new St.Label({
        text: displayName,
        style_class: 'fuhgawz-user-card-name',
        x_align: Clutter.ActorAlign.CENTER,
      });
