 ...
 * recentItemsSubmenu.js - Manages the recent items submenu in the system menu.
 */
...
export const RecentItemsSubmenu = GObject.registerClass(
  ...
  _scheduleOpen() {
    ...
    GLib.Source.set_name_by_id(this._openDelayTimeoutId, 'FUHGlobeRecentOpenDelay');
  }
  ...
  async _ensureRecentMenu() {
    ...
    this._recentMenu.actor.add_style_class_name('fuhgawz-recent-menu');
  ...
  _notifyLaunchFailure(title, message) {
    try {
      ...
      logError(error, 'Failed to display system menu notification');
    }
  }
