 ...
 * the extension with `gjs -m aboutWindow.js`. Reads hardware and OS details
...
const CSS = `
label.fuhgawz-about-title {
  font-size: 22px;
  font-weight: 700;
}
label.fuhgawz-about-key {
  font-weight: 600;
}
label.fuhgawz-about-value {
  font-weight: 500;
}
grid.fuhgawz-about-grid label,
label.fuhgawz-about-subtitle {
  font-size: 13px;
}
button.fuhgawz-about-button {
  font-weight: 500;
  font-size: 13px;
  min-height: 0;
  padding: 4px 14px;
...
Gettext.bindtextdomain('fuhgawzglbmenu', GLib.build_filenamev([extensionDir, 'locale']));
Gettext.textdomain('fuhgawzglbmenu');
...
function buildWindow(application) {
  ...
  titleLabel.add_css_class('fuhgawz-about-title');
  ...
  grid.add_css_class('fuhgawz-about-grid');
  details.forEach(([key, value], row) => {
    const keyLabel = new Gtk.Label({ label: key, xalign: 1 });
    keyLabel.add_css_class('fuhgawz-about-key');
    ...
    valueLabel.add_css_class('fuhgawz-about-value');
  ...
  if (vendor) {
    ...
    subtitleLabel.add_css_class('fuhgawz-about-subtitle');
...
const application = new Adw.Application({
  application_id: 'com.github.katon26.fuhgawzglbmenu.About',
});
