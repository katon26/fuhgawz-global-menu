 ...
 * Launched by the extension with `gjs -m forceQuitWindow.js`. Gets the list of
...
const IFACE = 'org.gnome.Shell.Extensions.FUHGlobeGlobalMenu.ForceQuit';

const CSS = `
list.fuhgawz-fq-list > row {
  min-height: 0;
  padding: 2px 8px;
}
list.fuhgawz-fq-list > row:selected {
  background-color: @accent_bg_color;
  color: @accent_fg_color;
}
button.fuhgawz-fq-button {
  min-height: 0;
  padding: 4px 14px;
}
`;
...
async function refresh() {
  ...
  try {
    ...
    logError(error, 'Failed to fetch running applications from the system menu');
    return;
  }
...
function buildWindow(application) {
  ...
  listBox.add_css_class('fuhgawz-fq-list');
  ...
  forceQuitButton.add_css_class('fuhgawz-fq-button');
...
const application = new Adw.Application({
  application_id: 'com.github.katon26.fuhgawzglbmenu.ForceQuit',
});
